...
SELECT producer, director, Sum(release_date) FROM movies group BY Lead_actor, director 
having sum(release_date) > 12/02/2015;
SELECT CONCAT(producer, ' ', Lead_actor) as full_names, sum(Awards) FROM movies GROUP BY producer
...
CASE WHEN Grarne LIKE '%Action%' THEN 'Action_movies' WHEN Lead_actor = 'Salma Khan' OR 
Lead_actor = 'Aamir Khan' THEN 'Lead_actor' ELSE 'None' END AS report FROM movies;    -- important
...
SELECT film_name, author_lname FROM books WHERE author_lname LIKE 'C%' OR author_lname LIKE 'S%';  -- Check Again
SELECT film_name, author_lname FROM books WHERE SUBSTR(author_lname,1,1) = 'C' OR SUBSTR(author_lname,1,1) = 'S';  -- Check Again
SELECT film_name, author_lname FROM books WHERE SUBSTR(author_lname,1,1) IN ('C', 'S');  -- Check Again
...
SELECT name, birthdt FROM people WHERE birthdt BETWEEN '1980-01-01' AND '2000-01-01';  -- important
SELECT name, birthdt FROM people WHERE birthdt BETWEEN 1980-01-01 AND 2000-01-01;
SELECT name, birthdt FROM people WHERE birthdt BETWEEN CAST('1980-01-01' AS DATETIME) AND CAST('2000-01-01' AS DATETIME); -- important
